...
\end{enumerate}
  ...
  \begin{equation}
      \frac{cE(0,t)}{4} + \frac{F_0(t)}{2} = \sigma_{sb} T_{bath}^4 \implies F_0 = 2\sigma_{sb} T_{bath}^4 - \frac{cE(0,t)}{2}
  \end{equation}
  ...
  \begin{equation}
      \frac{cE(L,t)}{4} - \frac{F_L(t)}{2} = 0 \implies F_L = \frac{cE(L,t)}{2}
  \end{equation}
  
  \subsection{Discretization in Lagrangian Coordinates}
  In our staggered grid, cell centers are indexed by $j=1 \dots N$ and faces by $i=0 \dots N$. We assume the face values of energy density $E$ are approximately equal to the adjacent cell centers ($E_0 \approx E_1$ and $E_{face, N} \approx E_N$). Substituting the boundary fluxes into the energy balance equation for the first and last cells:
  
  \paragraph{First Cell ($j=1$):}
  \begin{equation}
      \frac{E_1^{n+1} - E_1^n}{\Delta t} + \frac{\rho_1^*}{\Delta m_1} \left( F_1^{n+1} - \left[ 2 \sigma_{sb} T_{bath}^4 - \frac{c E_1^{n+1}}{2} \right] \right) = S_1^{n+1}
  \end{equation}
  Using $F_1^{n+1} = -\rho_1^* D_1^n \frac{E_2^{n+1} - E_1^{n+1}}{\Delta m_1}$, we group terms to find the modified coefficients:
  \begin{gather}
      \boxed{\tilde{b}_1^n = b_1^n + \frac{c \rho_1^*}{2 \Delta m_1}} \\
      \boxed{\tilde{d}_1^n = d_1^n + \frac{2 \rho_1^* \sigma_{sb} T_{bath}^4}{\Delta m_1}}
  \end{gather}
  
  \paragraph{Last Cell ($j=N$):}
  \begin{equation}
